...
class BaseEagle3Drafter(nn.Module, ABC):
    ...
    def __init__(
        ...
        self.tokenizer = None  # To be set later if needed for visualization
    ...
    def _finalize_results(
        ...
        for i, path in enumerate(retrieve_indices.tolist()):
            ...
            for idx in clean_path_indices:
                if idx == 0:
                    path_str_parts.append(f"ROOT({decode_tok(root_token_id)})")
                else:
                    token_id = draft_tokens[idx].item()
                    # idx-1 because draft_scores doesn't include root (which is at index 0 of draft_tokens)
                    # wait, draft_tokens = [sample_token, all_tokens[top_indices]]
